...
!!! note
    ...
    A default ClusterTemplate named default-cluster-template is already provisioned in the environment and can be used to create new clusters.
    If you choose to use the default template, you can skip the creation of a custom ClusterTemplate. To do so, pass the --cluster-template
    default-cluster-template parameter during cluster creation.
